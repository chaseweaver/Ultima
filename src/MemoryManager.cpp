...
void MemoryManager::coalesce(int memory_handle)
{
	...
	do
	{
		...
		if((tmp_node_2->limit == tmp_node->base - 1) && (tmp_node_2->status == HOLE)){
			node_found_2 = true;
			if(tmp_node_2->handle == -1){
				with_original_block = true;
			}
		}
...
std::string MemoryManager::memory_dump()
{
	...
	{
		MEMORY_NODE *mem = tmp->dequeue();
		str += std::to_string(mem->handle) + " @ " + std::to_string(mem->limit - mem->base) + " | " + memory_dump(mem->base, mem->limit - mem->base) + "\n";
	} while (!tmp->empty());
...
std::string MemoryManager::memory_dump_mem()
{
	...
	str += "Main Memory @ " + std::to_string(memory_core->memory_size()) + " | " + memory_core->memory_dump() + "\n";
